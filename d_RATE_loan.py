...
def RATE_loan():
    ...
    if option_check==True:
        image=Image.open("./RATE_loan.png")
        st.write("借入金の調達金利を求める。\n毎月15万円づつ25年間で返済していく。\n借入金は何％以下で借りればよいか？")
        st.image(image,caption='RATE（借入金調達金利計算）',use_column_width=True)
    ...
    pmt=st.sidebar.text_input('取崩し額（月）<マイナス入力>','-150000')
    pv=st.sidebar.text_input('現在の年金原資額','30000000')
