...
def NPER_pension():
    ...
    if option_check==True:
        image=Image.open("./NPER_pen.png")
        st.write("年金原資3千万円の取崩し可能年数を算出する。\n毎月15万円づつ取り崩すには何年まで可能か？\n利率2％で運用するものとする。")
    ...
    pmt=st.sidebar.text_input('取崩額（月額）','150000')
    pv=st.sidebar.text_input('年金原資額<マイナス入力>','-30000000')
    fv=st.sidebar.text_input('将来年金原資残額','0')
