...
def CUMPRINC_loan():
    ...
    # メインウィンドウを作成
    app_cumprinc_l = tk.Toplevel()
    # ウィンドウのサイズを設定
    app_cumprinc_l.geometry('450x350+500+100')
    # 画面タイトル
    app_cumprinc_l.title('指定期間返済元金合計計算')

    #--テキストボックス-------------------------------------------------------------------------
    # ラベル
    label1 = tk.Label(app_cumprinc_l,text='利率（年利）')
    label1.place(x=30, y=20)

    label2 = tk.Label(app_cumprinc_l,text='返済期間（年）')
    label2.place(x=30, y=70)

    label3 = tk.Label(app_cumprinc_l,text='借入額')
    label3.place(x=30, y=120)

    label4 = tk.Label(app_cumprinc_l,text='計算開始_回目(月)')
    label4.place(x=30, y=170)

    label5 = tk.Label(app_cumprinc_l,text='計算終了_回目(月)')
    label5.place(x=30, y=220)

    label6 = tk.Label(app_cumprinc_l,text='支払期日(0:期末, 1:期首)')
    label6.place(x=30, y=270)

    # テキストボックス
    textBox1 = tk.Entry(app_cumprinc_l)
    textBox1.insert(0,"0.015")
    textBox1.place(x=250, y=20)

    textBox2 = tk.Entry(app_cumprinc_l)
    textBox2.insert(1,"20")
    textBox2.place(x=250, y=70)

    textBox3 = tk.Entry(app_cumprinc_l)
    textBox3.insert(2,"30000000")
    textBox3.place(x=250, y=120)

    textBox4 = tk.Entry(app_cumprinc_l)
    textBox4.insert(3,"1")
    textBox4.place(x=250, y=170)

    textBox5 = tk.Entry(app_cumprinc_l)
    textBox5.insert(4,"120")
    textBox5.place(x=250, y=220)

    textBox6 = tk.Entry(app_cumprinc_l)
    textBox6.insert(5,"1")
    ...
    def exp():
        ...
        exp="{}円を金利{}%で借入をした。\n{}年で完済する場合の指定期間の返済元金合計額を求める".format(pv,rate,nper)
        tk.messagebox.showinfo(title="指定期間返済元金合計額", message=exp)
    ...
    def fv_gr():
        ...
        #cumprinc=npf.cumprinc(rate/12,nper*12,pv,speriod,eperiod,when)
        #cumprinc=int(cumprinc)
    ...
    def val():
        ...
        cumprinc=cumprinc*-1
        #数値を3桁区切りにする
        cumprinc="{:,}".format(cumprinc)
        print('指定期間支払元金合計額 CUMPRINC:',cumprinc)

        #回答をメッセージボックスで表示する
        res="{}回目から{}回目の支払元金合計額は{}円です".format(speriod,eperiod,cumprinc)
        tk.messagebox.showinfo(title="指定期間支払元金合計額", message=res)
        #msg.showinfo("必要積立月額",cumipmt)

    #--ボタンの作成と配置---------------------------------------------------------------------
    #-------------------------------------------------------------------------------------
    # クリック時にExP()関数を呼ぶ
    button3 = tk.Button(app_cumprinc_l,text = '要件確認',command =exp).place(x=30, y=310)
    #---------------------------------------------------------------------------------------
    # イメージ図の作成と配置
    # クリック時にimg()関数を呼ぶ
    #button4 = tk.Button(app_cumprinc_l,text = 'イメージ図',command =img).place(x=100, y=310)
    #-----------------------------------------------------------------------------------------
    # グラフボタンの作成と配置
    # クリック時にFV_gr()関数を呼ぶ
    button2 = tk.Button(app_cumprinc_l,text = 'グラフ',width=5,command =fv_gr).place(x=320, y=310)
    #---------------------------------------------------------------------------------------
    # OKボタンの作成と配置
    # クリック時にval()関数を呼ぶ
    button1 = tk.Button(app_cumprinc_l,text = 'OK',width=5,command = val).place(x=390, y=310)
    #---------------------------------------------------------------------------------------

    #app_cumprinc_l.mainloop()
#--------------------------------------------------------------------------------------------------
