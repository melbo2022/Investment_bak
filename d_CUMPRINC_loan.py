...
def CUMPRINC_loan():
    ...
    if option_check==True:
        image=Image.open("./CUMPRINC_loan.png")
        st.write("3千万円を金利1.5%で借入をした。\n20年で完済する場合の指定期間の返済元金合計額を求める")
        st.image(image,caption='CUMPRINC（指定期間借入返済元金合計額計算）',use_column_width=True)
    ...
    if submit_btn:
        ...
        #cumprincがnumpyで使用できないためipmtより計算する
        rate=float(rate)
        nper=int(nper)
        pv=int(pv)
        speriod=int(speriod)
